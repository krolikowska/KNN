...
//
// summary:	Implements the recommendations controller class
///-------------------------------------------------------------------------------------------------
...
{
    /// <summary>   A controller for handling recommendations. </summary>
    public class RecommendationsController : ApiController
    {
        private IDataManager _dm;
        private IUserBasedCollaborativeFiltering _runner;
        ...
        {
            return _dm.GetRecomenndedBooksForUser(userId).Select(x => new BookModel()
            {
